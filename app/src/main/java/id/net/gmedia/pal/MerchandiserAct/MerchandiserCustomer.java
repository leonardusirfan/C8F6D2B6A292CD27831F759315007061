...
public class MerchandiserCustomer extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_merchandiser_customer);
        ...
        ApiVolleyManager.getInstance().addRequest(this, Constant.URL_CUSTOMER_REGIONAL + parameter, ApiVolleyManager.METHOD_GET,
                Constant.getTokenHeader(AppSharedPreferences.getId(this)), new AppRequestCallback(new AppRequestCallback.AdvancedRequestListener() {
                        ...
                        try{
                            ...
                            for(int i = 0; i < customer.length(); i++){
                                listCustomer.add(new CustomerModel(customer.getJSONObject(i).getString("kode_pelanggan"),
                                        ...
                                        customer.getJSONObject(i).getString("kota"), customer.getJSONObject(i).getDouble("latitude"),
                                        customer.getJSONObject(i).getDouble("longitude"), customer.getJSONObject(i).getString("status")));
                                loaded += 1;
                            }
                            ...
                            Toast.makeText(MerchandiserCustomer.this, R.string.error_json, Toast.LENGTH_SHORT).show();
                    ...
                    public void onFail(String message) {
                        Toast.makeText(MerchandiserCustomer.this, message, Toast.LENGTH_SHORT).show();
                        AppLoading.getInstance().stopLoading();
                        loadMoreScrollListener.finishLoad();
                    }
                }));
